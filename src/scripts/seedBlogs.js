...
async function seed() {
  ...
  for (const blog of blogs) {
    const { data, error } = await supabase.from('blogs').upsert(blog);
    if (error) {
      console.error('Error inserting blog:', blog.id, error.message);
    } else {
      console.log('Successfully inserted/updated blog:', blog.id);
    }
  }
